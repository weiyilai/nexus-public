    ...
    extends TestSupport
{
  ...
  @Test
  public void testConvert_withMixedVersionPolicy() {
    MavenGroupRepositoryApiRequest request = createRequest("MIXED", "STRICT", "ATTACHMENT");
    Configuration config = underTest.convert(request);

    assertThat(config, notNullValue());
    assertThat(config.attributes("maven").get("versionPolicy"), is("MIXED"));
    assertThat(config.attributes("maven").get("layoutPolicy"), is("STRICT"));
    assertThat(config.attributes("maven").get("contentDisposition"), is("ATTACHMENT"));
  }

  @Test
  public void testConvert_withReleaseVersionPolicy() {
    MavenGroupRepositoryApiRequest request = createRequest("RELEASE", "PERMISSIVE", "INLINE");
    Configuration config = underTest.convert(request);

    assertThat(config, notNullValue());
    assertThat(config.attributes("maven").get("versionPolicy"), is("RELEASE"));
    assertThat(config.attributes("maven").get("layoutPolicy"), is("PERMISSIVE"));
    assertThat(config.attributes("maven").get("contentDisposition"), is("INLINE"));
  }

  @Test
  public void testConvert_withSnapshotVersionPolicy() {
    MavenGroupRepositoryApiRequest request = createRequest("SNAPSHOT", "STRICT", "ATTACHMENT");
    Configuration config = underTest.convert(request);

    assertThat(config, notNullValue());
    assertThat(config.attributes("maven").get("versionPolicy"), is("SNAPSHOT"));
    assertThat(config.attributes("maven").get("layoutPolicy"), is("STRICT"));
    assertThat(config.attributes("maven").get("contentDisposition"), is("ATTACHMENT"));
  }
