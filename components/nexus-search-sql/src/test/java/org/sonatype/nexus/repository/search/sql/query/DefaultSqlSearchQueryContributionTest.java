    ...
    extends TestSupport
{
  ...
  public void testSplit() {
    ...
    // escaped wildcards - these should remain as LenientTerm now due to our change
    assertThat(split("foo\\*bar"), contains(new LenientTerm("foo*bar")));
    assertThat(split("foo\\?bar"), contains(new LenientTerm("foo?bar")));
    assertThat(split("foo\\*"), contains(new LenientTerm("foo*")));
    assertThat(split("foo\\?"), contains(new LenientTerm("foo?")));
  ...
  public void testCaseInsensitiveGroupSearch() {
    // Test uppercase letters are handled as LenientTerm for case-insensitive search
    assertThat(split("Support"), contains(new LenientTerm("Support")));
    assertThat(split("TEAM"), contains(new LenientTerm("TEAM")));
    assertThat(split("Maven"), contains(new LenientTerm("Maven")));
  }

  @Test
  public void testHierarchicalGroupNames() {
    // Test hierarchical group names with slashes and mixed case
    assertThat(split("/Support/Team"),
        contains(new LenientTerm("/Support/Team")));
    assertThat(split("org/apache/maven"),
        contains(new LenientTerm("org/apache/maven")));
  }
  ...
  public void testLenientMatchField() {
    // Test that group.raw field (which has exactMatch=false) still creates LenientTerm
    Collection<StringTerm> result = splitForField(GROUP_RAW, "support");
    assertThat(result, contains(new LenientTerm("support")));
  }
