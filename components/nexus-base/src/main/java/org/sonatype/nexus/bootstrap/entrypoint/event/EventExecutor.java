...
import static org.sonatype.nexus.bootstrap.entrypoint.event.EventManagerImpl.HOST_THREAD_POOL_SIZE;
    ...
    implements Executor
{
  ...
  public EventExecutor(
      ...
      @Value("${nexus.event.affinityTimeout:1s}") final Duration affinityTimeout,
      @Value("${nexus.event.singleCoordinator:false}") final boolean singleCoordinator,
      @Value("${nexus.event.fairThreading:false}") final boolean fairThreading)
  {
    this.affinityEnabled = affinityEnabled;
    this.affinityCacheSize = affinityCacheSize;
    this.affinityTimeout = Time.seconds(checkNotNull(affinityTimeout).getSeconds());
