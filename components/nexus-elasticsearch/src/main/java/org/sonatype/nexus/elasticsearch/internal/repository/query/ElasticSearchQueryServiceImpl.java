 ...
 */
package org.sonatype.nexus.elasticsearch.internal.repository.query;
...
import org.sonatype.nexus.elasticsearch.internal.repository.query.SearchSubjectHelper.SubjectRegistration;
import org.sonatype.nexus.elasticsearch.internal.repository.selector.ContentAuthPluginScriptFactory;
    ...
    implements ElasticSearchQueryService
{
  ...
  private SearchResponse executeSearch(
      final RepositoryQueryBuilder repoQuery,
      final String[] searchableIndexes,
      final int from,
      final int size,
      @Nullable final QueryBuilder postFilter)
  {
    SearchRequestBuilder searchRequestBuilder = client.get()
        .prepareSearch(searchableIndexes)
        .setTypes(TYPE)
        .setQuery(repoQuery)
        .setFrom(from)
        .setSize(size)
  ...
  private boolean validateQuery(final QueryBuilder query) {
    ...
    try {
      ...
      if (!validateQueryResponse.isValid()) {
        if (log.isDebugEnabled()) {
          String explanations = validateQueryResponse.getQueryExplanation()
              .stream()
      ...
      implements Iterator<SearchHit>
  {
    ...
    public boolean hasNext() {
      ...
      if (response == null) {
        SearchRequestBuilder builder = client.get()
            .prepareSearch(searchableIndexes)
            .setTypes(TYPE)
            .setQuery(query)
        ...
        SearchScrollRequestBuilder builder = client.get()
