 ...
 */
package org.sonatype.nexus.internal.kv.upgrade;
...
/**
 * Adds indexes to nexus_key_value table for existing databases.
 *
...
public class NexusKeyValueIndexesMigrationStep_2_42
    implements DatabaseMigrationStep
{
  private static final String TABLE_NAME = "nexus_key_value";

  @Override
  public Optional<String> version() {
    return Optional.of("2.42");
  }
  ...
  private void createIndexes(final Connection connection) throws SQLException {
    ...
    String columnName = isH2(connection) ? "`key`" : "key";

    // Create unique index first
    createUniqueIndexIfNotExists(connection, "uk_nexus_key_value_key",
        TABLE_NAME, columnName);

    // Create regular index
    createIndexIfNotExists(connection, "idx_nexus_key_value_key",
        TABLE_NAME, columnName);
  }
