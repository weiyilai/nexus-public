...
public interface SecurityConfiguration
{
  ...
  /**
   * Get all roles that contain a specific privilege.
   * Default implementation iterates through all roles (O(n)), but implementations
   * with a reverse index can provide O(1) lookups.
   *
   * @param privilegeId the privilege ID to search for
   * @return set of role IDs that contain this privilege
   */
  default Set<String> getRolesByPrivilege(String privilegeId) {
    return getRoles().stream()
        .filter(role -> role.getPrivileges().contains(privilegeId))
        .map(CRole::getId)
