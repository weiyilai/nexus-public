...
public class ValidatingComponentUpload
{
  ...
  private void validateAllowedComponentFields() {
    collectNotAllowedFields(componentUpload.getFields(), getAllowedComponentFields())
        .forEach(field -> validationErrorsException.withError(field, format("Unknown component field '%s'", field)));
  }
  ...
  private void validateAllowedAssetFields(final AssetUpload asset, final int assetIndex) {
    collectNotAllowedFields(asset.getFields(), getAllowedAssetFields()).forEach(field -> validationErrorsException
        .withError(field, format("Unknown field '%s' on asset '%s'", field, assetIndex)));
  }
  ...
  private Collection<String> collectNotAllowedFields(
  ...
  private Set<String> getAllowedAssetFields() {
    return uploadDefinition.getAssetFields()
        .stream()
  ...
  private Set<String> getAllowedComponentFields() {
    return uploadDefinition.getComponentFields()
        .stream()
        .map(UploadFieldDefinition::getName)
        .collect(Collectors.toSet());
  }
