    ...
    extends TestSupport
{
  ...
  protected abstract UploadHandler newRawUploadHandler(
  ...
  public void testGetDefinitionWithExtensionContributions() {
    ...
    underTest =
        newRawUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(), getDefinitionExtensions());
    UploadDefinition def = underTest.getDefinition();

    assertThat(def.getComponentFields(),
        contains(
            field("directory", "Directory", "Destination for uploaded files (e.g. /path/to/files/)", false, STRING,
  ...
  protected UploadFieldDefinition field(
      final String name,
  ...
  protected static class TestUploadDefinitionExtension
